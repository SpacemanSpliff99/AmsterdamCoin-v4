...
- [Building PuffsCoin](#building-puffscoin)
...
Clone the git repositories for puffscoin and gitian and then checkout the puffscoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the puffscoin repository
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/puffscoin/.git/
...
./bin/gbuild --commit puffscoin=${COMMIT} --url puffscoin=${URL} ../puffscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/puffscoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/puffscoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/puffscoin-build.assert
```
